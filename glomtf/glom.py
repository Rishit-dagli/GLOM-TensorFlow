...
class Glom(tf.keras.Model):
    def __init__(
        ...
        self.image_to_tokens = tf.keras.Sequential([Rearrange('b c (h p1) (w p2) -> b (h w) (p1 p2 c)',
                                                              p1=patch_size,
                                                              p2=patch_size),
        ...
        self.attention = ConsensusAttention(num_patches_side,
    ...
    def call(self, img, iters=None, levels=None, return_all=False):
        ...
        for _ in range(iters):
            ...
            # [[0, 1],[0, 0]]
            top_down_out = tf.pad(top_down_out, [[0, 0], [0, 0], [0, 1], [0, 0]])
            ...
            rearranged_num_contributions = tf.cast(rearrange(num_contributions, 'l -> () () l ()'), tf.float32)
